...
def compile(compiler, args):
    ...
    parts = arg_str.split()
    if len(parts) < 2:
        raise Exception(f"MUL: Za malo parametrow: {args}")
    dest_str = parts[0].upper()
    src_str  = parts[1].upper()
    reg_map = {'AX': 0, 'BX': 1, 'CX': 2, 'DX': 3, 'SP': 4, 'BP': 5}
    if dest_str not in reg_map:
        raise Exception(f"MUL: Pierwszy argument musi byc rejestrem: {dest_str}")
    ...
    if src_str in reg_map:
        ...
        if src_raw in compiler.labels:
            ...
            try:
                ...
                raise Exception(f"MUL: Nieznana wartosc '{src_raw}'")
    # Opcode 12
    return struct.pack('>B B H I B', 12, mode, dest, src, 0)
